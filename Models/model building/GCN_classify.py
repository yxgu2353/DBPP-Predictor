...
def load_data(data) -> 'DGLGraph Form':
    ...
    dataset = MoleculeCSVDataset(df=data,
                                 ...
                                 cache_file_path='result/GCN/DGLGraph.bin',
                                 n_jobs=-1,
                                 init_mask=True)
    return dataset
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    Train_performance.to_csv('result/GCN/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    Valid_performance.to_csv('result/GCN/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        ...
        Test_performance.to_csv('result/GCN/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    ...
    # model setting
    model = model_zoo.GCNPredictor(in_feats=n_feats,
                                   hidden_feats=[256],
                                   dropout=[0.210])
    model = model.to(device)
    loss_criterion = nn.BCEWithLogitsLoss(reduction='none')
    optimizer = Adam(model.parameters(), lr=0.0003, weight_decay=0.002)
    stopper = EarlyStopping(patience=50, filename='result/GCN/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv('result/GCN/Training_loss_auc.csv')

    # Save model
    fn = 'result/GCN/GCN_model.pt'
    ...
    with open('result/GCN/test_result.txt', 'w') as f:
