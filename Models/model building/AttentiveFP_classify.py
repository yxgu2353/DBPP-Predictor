...
# Feature initialize
node_feature = AttentiveFPAtomFeaturizer(atom_data_field='hv')
bond_feature = AttentiveFPBondFeaturizer(bond_data_field='he')
...
def load_data(data) -> 'DGLGraph Form':
    ...
    dataset = MoleculeCSVDataset(df=data,
                                 ...
                                 cache_file_path='result/AttetntiveFP/DGLGraph.bin',
                                 n_jobs=-1,
                                 init_mask=True)
    return dataset
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    Train_performance.to_csv('result/AttetntiveFP/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    Valid_performance.to_csv('result/AttetntiveFP/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        ...
        Test_performance.to_csv('result/AttetntiveFP/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    ...
    optimizer = Adam(model.parameters(), lr=0.003, weight_decay=4.357976043245693e-05)
    stopper = EarlyStopping(patience=30, filename='result/AttetntiveFP/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv('result/AttetntiveFP/Training_loss_auc.csv')

    # Save model
    fn = 'result/AttetntiveFP/AttentiveFP_model.pt'
    ...
    for batch_id, batch_data in enumerate(test_loader):
        ...
        n_feats = bg.ndata.pop('hv').to(device)
        e_feats = bg.edata.pop('he').to(device)
        logits = torch.sigmoid(best_model(bg, n_feats, e_feats))
    ...
    with open('result/AttetntiveFP/test_result.txt', 'w') as f:
