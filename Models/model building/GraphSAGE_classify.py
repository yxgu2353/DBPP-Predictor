...
# Loading dataset
def load_data(data, path) -> 'DGLGraph Form':
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    Train_performance.to_csv('result/GraphSAGE/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    Valid_performance.to_csv('result/GraphSAGE/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        ...
        Test_performance.to_csv('result/GraphSAGE/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    ...
    optimizer = Adam(model.parameters(), lr=0.088, weight_decay=0.0002)
    stopper = EarlyStopping(patience=50, filename='result/GraphSAGE/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv('result/GraphSAGE/Training_loss_auc.csv')

    # Save model
    fn = 'result/GraphSAGE/GraphSAGE_model.pt'
    ...
    with open('result/GraphSAGE/test_result.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    train_dc = pd.read_csv('Train_Sample01_1.csv', low_memory=False)
    valid_dc = pd.read_csv('Test_Sample01_1.csv', low_memory=False)
    test_dc = pd.read_csv('Valid_Sample01.csv', low_memory=False)

    feat_columns = train_dc.drop(['SMILES', 'label'], axis=1)
    feat_size = feat_columns.columns.tolist()
    feature_size = len(feat_size)

    train_datasets = load_data(train_dc, 'result/GraphSAGE/train')
    valid_datasets = load_data(valid_dc, 'result/GraphSAGE/valid')
    test_datasets = load_data(test_dc, 'result/GraphSAGE/test')
